...
module.exports = {
    ...
    notifyLikes: async (
        ...
        if (message.count <= 0 && action === "remove") {
            ...
            // Substract count from notifications
        ...
        // Check if notifyUser doesn't already habe a notification associated with this post
        const hasNotification = notifyUser.notifications.content.filter(item => {
            if (item.payload.originalId) {
                if (item.payload.originalId.toString() === postId.toString() && item.payload.alertType === "like") {
                    return item
                }
            }
        })
    ...
    notifyComment: async (
        ...
        if (type === "comment" || type === "post") messageString = "commented to"
    ...
    notifyFriend: async (currentUser, requestingUser, type) => {
        ...
        if (requestingUserNotification.length <= 0) {
            ...
            // Add count to notification
            requestingUser.notifications.count = requestingUser.notifications.count + 1
        }
    }
}
...
function generateMessage(updatedPost, action, string) {
    ...
    if (string === "like") {
        ...
        if (string === "commented to") property = "comments"
    ...
    if (count === 2) {
        ...
        message += `${user[count - 2].firstName} ${user[count - 2].lastName} ${tring} your post`
    } else if (count > 2) {
        message += `${user[count - 1].firstName} ${user[count - 1].lastName}, `
        message += `${user[count - 2].firstName} ${user[count - 2].lastName} and `
        message += `${count - 2} others ${string} your post`
    } else {
        message += `${user[count - 1].firstName} ${user[count - 1].lastName} ${string} your post`
    }
